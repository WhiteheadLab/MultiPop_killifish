...
###Merge expsn and p-values, add annotations
```{r}
...
trtavg <- as.data.frame(tmv)
trtavg <- cbind("Gene"=rownames(trtavg), data.frame(trtavg, check.names=F, row.names=NULL))

adj_p_trtavg <- merge(adj_pval_all, trtavg, by.x = "Gene")
...
names(genemodels) <- c("Gene", "name", "cons", "lib")


adj_p_ann <- merge(adj_p_trtavg, genemodels, by.x = "Gene", all.x = TRUE)
write.table(adj_p_ann, "~/Documents/NIEHS_LSU_UCD/niehs/multipop/200427_multipop_doseresponse_fdr_adj_trtavg_anns.csv", col.names=TRUE, row.names=FALSE, sep=",") 
...
adj_p_trtnorm <- read.csv("~/Documents/NIEHS_LSU_UCD/niehs/multipop/191101_fdradj_pval_trtnorm_counts.csv")
adj_p_gnorm <- read.csv("~/Documents/NIEHS_LSU_UCD/niehs/multipop/191101_fdradj_pval_gnorm_counts.csv")
...
expos_main <- pheatmap(exposure[,-(1:4)], 
                       color = colorRampPalette(c("turquoise1", "black", "yellow"))(50), 
                       breaks=seq(from=-2, to=2, length.out=51), 
                       clustering_distance_rows="correlation", 
                       cluster_cols=FALSE, 
                       cutree_rows = 2, 
                       show_rownames = FALSE, 
                       labels_col=c("LA-REF: 0%", "LA-REF: 10%", "LA-REF: 32%", "LA-REF: 56%", 
                                "LA-DWH: 0%", "LA-DWH: 10%", "LA-DWH: 32%", "LA-DWH:56%",
                                "TX-REF: 0%", "TX-REF: 10%", "TX-REF: 32%", "TX-REF: 56%",
                                "TX-SF: 0%", "TX-SF: 10%", "TX-SF: 32%", "TX-SF: 56%"),
                       main= "Main effect of Embryonic Exposure", gaps_col = c(4,8,12))
...
popexp <- pheatmap(ixn[,-(1:4)], 
                   ...
                   labels_col=c("LA-REF: 0%", "LA-REF: 10%", "LA-REF: 32%", "LA-REF: 56%", 
                                ...
                                "TX-REF: 0%", "TX-REF: 10%", "TX-REF: 32%", "TX-REF: 56%",
                                "TX-SF: 0%", "TX-SF: 10%", "TX-SF: 32%", "TX-SF: 56%"),
                   main= "Interaction effect of Population and Embryonic Exposure", 
                   gaps_col = c(4,8,12), border_color = NA)
...
ixn.clust <- as.data.frame(cbind(ixn2$Gene, cluster = cutree(popexp$tree_row, k = 2)))
ixn.list <- list()
...
pop <- adj_p_gnorm[adj_p_gnorm$cof =="pop" & 
                     ...
                     !(adj_p_gnorm$Gene %in% exposure$Gene), ]

pop_main <- pheatmap(pop[,-(1:4)], 
                     color = colorRampPalette(c("turquoise1", "black", "yellow"))(50), 
                     ...
                     labels_col=c("LA-REF: 0%", "LA-REF: 10%", "LA-REF: 32%", "LA-REF: 56%", 
                                ...
                                "TX-REF: 0%", "TX-REF: 10%", "TX-REF: 32%", "TX-REF: 56%",
                                "TX-SF: 0%", "TX-SF: 10%", "TX-SF: 32%", "TX-SF: 56%"),
                     main= "Main effect of Population", gaps_col = c(4,8,12))

pop2 <- pop[pop_main$tree_row$order,]

pop_main <- pheatmap(pop2[,-(1:4)], 
                     color = colorRampPalette(c("turquoise1", "black", "yellow"))(50), 
                     breaks=seq(from=-1, to=1, length.out=51), 
                     cluster_cols=FALSE, 
                     clustering_distance_rows="correlation", 
...
pop.clust <- as.data.frame(cbind(pop2$Gene, cluster = cutree(pop_main$tree_row, k = 3)))
pop.list <- list()
...
david[is.na(david)] <- " " 
write.table(david, file="~/Documents/NIEHS_LSU_UCD/niehs/multipop/200421_clustergenelists.txt", row.names = FALSE, col.names = TRUE, sep="\t", quote=FALSE)
...
ahr_genes <- as.data.frame(read_xlsx("~/Documents/NIEHS_LSU_UCD/niehs/multipop/ahr_target_gene_models.xlsx"))

ahr_genes <- ahr_genes[,1:4]
colnames(ahr_genes)[4] <- "Gene Annotation"

# Subset counts value by AHR pathway-related genes
...
# subset DEGs with significance thresholds
...
ahr_genes <- as.data.frame(read_xlsx("~/Documents/NIEHS_LSU_UCD/niehs/multipop/ahr_target_gene_models.xlsx"))
ahr_genes <- as.data.frame(read_xlsx("~/Documents/NIEHS_LSU_UCD/niehs/multipop/extended_ahr_gene_models.xlsx"))

ahr_genes <- ahr_genes[,1:4]
colnames(ahr_genes)[4] <- "Gene Annotation"

# declutter table so it includes 1 of each unique gene, counts for treatment averages for all ahr pathway-related genes present in background gene list
#all_ahr <- ahr_val[which(ahr_val$cof=="exposure"),]
all_ahr <- gnorm[gnorm$Gene %in% ahr_genes$`Killifish geneID`,]
all_ahr$annotation <- NA

for (i in 1:dim(all_ahr)[1]) {
  x <- all_ahr$Gene[i]
  all_ahr$annotation[i] <- ahr_genes$`Gene Annotation`[which(ahr_genes$`Killifish geneID`==x)]
}

rownames(all_ahr) <- c()

# heatmap and clustering using pearson correlation
all_ahr_hm <- pheatmap(all_ahr[,-c(1,18)], 
                       color = colorRampPalette(c("turquoise1", "black", "yellow"))(50), 
                       breaks=seq(from=-1, to=1, length.out=51), 
                       clustering_distance_rows="correlation", 
                       cluster_cols=FALSE, cutree_rows = 2,
                       show_rownames = FALSE,
                       labels_col=c("LA-REF: 0%", "LA-REF: 10%", "LA-REF: 32%", "LA-REF: 56%", 
                                "LA-DWH: 0%", "LA-DWH: 10%", "LA-DWH: 32%", "LA-DWH:56%",
                                "TX-REF: 0%", "TX-REF: 10%", "TX-REF: 32%", "TX-REF: 56%",
                                "TX-SF: 0%", "TX-SF: 10%", "TX-SF: 32%", "TX-SF: 56%"),
                       main= "AHR Pathway-Related Genes", gaps_col = c(4,8,12))


# only those with p-val < 0.1 for any term: 
ahr_val <- adj_p_counts[adj_p_counts$Gene %in% ahr_genes$`Killifish geneID`,]
ahr_sig <- ahr_val[ahr_val$adj_pvals <= 0.1,]
any_sig <- unique(ahr_sig$Gene)
all_sig_ahr <- all_ahr[all_ahr$Gene %in% any_sig,]

# heatmap and clustering using pearson correlation
all_sig_hm <- pheatmap(all_sig_ahr[,-c(1,18)], 
                       color = colorRampPalette(c("turquoise1", "black", "yellow"))(70), 
                       breaks=seq(from=-1, to=1, length.out=71), 
                       clustering_distance_rows="correlation", cluster_cols=FALSE, cutree_rows = 2,
                       labels_row= all_sig_ahr$annotation, 
                       labels_col=c("LA-REF: 0%", "LA-REF: 10%", "LA-REF: 32%", "LA-REF: 56%", 
                                "LA-DWH: 0%", "LA-DWH: 10%", "LA-DWH: 32%", "LA-DWH:56%",
                                "TX-REF: 0%", "TX-REF: 10%", "TX-REF: 32%", "TX-REF: 56%",
                                "TX-SF: 0%", "TX-SF: 10%", "TX-SF: 32%", "TX-SF: 56%"),
                       main= "AHR Pathway-Related Genes, adj. p-val < 0.1", gaps_col = c(4,8,12))
...
#curated from Reid_Appendix_S1_Gene_models (includes any/all cyp genes, ahr, arnt, tiparp, foxhead)
ahr_genes <- as.data.frame(read_xlsx("~/Documents/NIEHS_LSU_UCD/niehs/multipop/extended_ahr_gene_models.xlsx"))
ahr_genes <- as.data.frame(read_xlsx("~/Documents/NIEHS_LSU_UCD/niehs/multipop/ahr_target_gene_models.xlsx"))

#extract ahr related genes and set up dataframe
ahr_genes <- ahr_genes[,1:5]

#subset trtment group average dataframes by ahr genes and significant thresholds
ixn <- adj_p_trtnorm[adj_p_trtnorm$cof=="pop_exposure" & 
                       adj_p_trtnorm$adj_pvals <= 0.1,]

ahr_exp <- adj_p_trtnorm[adj_p_trtnorm$Gene %in% ahr_genes$`Killifish geneID` &
                           adj_p_trtnorm$cof =="exposure" & 
                           adj_p_trtnorm$adj_pvals <= 0.05 & 
                           !(adj_p_trtnorm$Gene %in% ixn$Gene),]

#add annotation column to subsetted trtnorm dataframe
ahr_exp$annotation <- NA

for (i in 1:dim(ahr_exp)[1]) {
  x <- ahr_exp$Gene[i]
  ahr_exp$annotation[i] <- ahr_genes$`Killifish gene annotation notes`[which(ahr_genes$`Killifish geneID`==x)]
}


# heatmap and clustering using pearson correlation
ahrexp <- pheatmap(ahr_exp[,-c(1:4,21)], 
                   color = colorRampPalette(c("turquoise1", "black", "yellow"))(50), 
                   breaks=seq(from=-1, to=1, length.out=51), 
                   clustering_distance_rows="correlation", 
                   cluster_cols=FALSE, cutree_rows = 2, 
                   labels_row= ahr_exp$annotation, 
                   labels_col=c("LA-REF: 0%", "LA-REF: 10%", "LA-REF: 32%", "LA-REF: 56%", 
                                "LA-DWH: 0%", "LA-DWH: 10%", "LA-DWH: 32%", "LA-DWH:56%",
                                "TX-REF: 0%", "TX-REF: 10%", "TX-REF: 32%", "TX-REF: 56%",
                                "TX-SF: 0%", "TX-SF: 10%", "TX-SF: 32%", "TX-SF: 56%"),
...
ahr_pop <- adj_p_gnorm[adj_p_gnorm$Gene %in% ahr_genes$`Killifish geneID` &
                           adj_p_gnorm$cof =="pop" & 
                           adj_p_gnorm$adj_pvals <= 0.05 & 
                           !(adj_p_gnorm$Gene %in% ixn$Gene),]

#add annotation column to subsetted trtnorm dataframe
ahr_pop$annotation <- NA

for (i in 1:dim(ahr_pop)[1]) {
  x <- ahr_pop$Gene[i]
  ahr_pop$annotation[i] <- ahr_genes$`Killifish gene annotation notes`[which(ahr_genes$`Killifish geneID`==x)]
}

#rownames(ahr_pop) <- ahr_pop$annotation

# heatmap and clustering using pearson correlation
ahrpop <- pheatmap(ahr_pop[,-c(1:4,21)], color = colorRampPalette(c("turquoise1", "black", "yellow"))(80), breaks=seq(from=-1, to=1, length.out=81), clustering_distance_rows="correlation", cluster_cols=FALSE, cutree_rows = 2, labels_row= ahr_pop$annotation, 
                   labels_col=c("LA-REF: 0%", "LA-REF: 10%", "LA-REF: 32%", "LA-REF: 56%", 
                                "LA-DWH: 0%", "LA-DWH: 10%", "LA-DWH: 32%", "LA-DWH:56%",
                                "TX-REF: 0%", "TX-REF: 10%", "TX-REF: 32%", "TX-REF: 56%",
                                "TX-SF: 0%", "TX-SF: 10%", "TX-SF: 32%", "TX-SF: 56%"),
...
#subset trtment group average dataframes by ahr genes and significant thresholds
ahr_ixn <- adj_p_trtnorm[adj_p_trtnorm$Gene %in% ahr_genes$`Killifish geneID` &
                           adj_p_trtnorm$cof =="pop_exposure" & 
                           adj_p_trtnorm$adj_pvals <= 0.1,]

ahr_ixn <- adj_p_gnorm[adj_p_gnorm$Gene %in% ahr_genes$`Killifish geneID` &
                           adj_p_gnorm$cof =="pop_exposure" & 
                           adj_p_gnorm$adj_pvals <= 0.1,]


#add annotation column to subsetted trtnorm dataframe
ahr_ixn$annotation <- NA

for (i in 1:dim(ahr_ixn)[1]) {
  x <- ahr_ixn$Gene[i]
  ahr_ixn$annotation[i] <- ahr_genes$`Killifish gene annotation notes`[which(ahr_genes$`Killifish geneID`==x)]
}
#rownames(ahr_ixn) <- ahr_ixn$annotation

# heatmap and clustering using pearson correlation
ahrixn <- pheatmap(ahr_ixn[,-c(1:4,21)], 
                   color = colorRampPalette(c("turquoise1", "black", "yellow"))(50), 
                   breaks=seq(from=-1, to=1, length.out=51), 
                   clustering_distance_rows="correlation", cluster_cols=FALSE, cutree_rows = 2, 
                   labels_row= ahr_ixn$annotation, 
                   labels_col=c("LA-REF: 0%", "LA-REF: 10%", "LA-REF: 32%", "LA-REF: 56%", 
                                "LA-DWH: 0%", "LA-DWH: 10%", "LA-DWH: 32%", "LA-DWH:56%",
                                "TX-REF: 0%", "TX-REF: 10%", "TX-REF: 32%", "TX-REF: 56%",
                                "TX-SF: 0%", "TX-SF: 10%", "TX-SF: 32%", "TX-SF: 56%"),
                   main= "Interaction of Population and Embryonic Exposure", gaps_col = c(4,8,12))
...
#p$trt <- y$samples$trt
pcadim <- dim(p)[2]-2
p.pca <- prcomp(p[,1:pcadim])
summary(p.pca)
p.pca.proportionvariances <- ((p.pca$sdev^2) / (sum(p.pca$sdev^2)))*100
p.PCi <- data.frame(p.pca$x, pop=p$pop, exposure=p$exposure)

q <- ggbiplot(p.pca, group=p$pop, labels=NULL, ellipse = TRUE, var.axes=FALSE) +
   geom_point(aes(colour=p$pop, shape=p$exp), size=3) + 
  scale_color_manual(name="Population", values=c("dodgerblue4", "deepskyblue1", "goldenrod1", "olivedrab3")) + 
  scale_shape_manual(name="Exposure", values=c(19,1,12,8)) + 
  scale_size(guide="none") +
  theme_minimal() +
  xlab(paste("PC1, ", round(p.pca.proportionvariances[1], 2), "%")) + 
  ylab(paste("PC2, ", round(p.pca.proportionvariances[2], 2), "%")) +
  labs(title="Average Expression Values in Treatment Groups \nExposure Main Effect") +
  theme(plot.title = element_text(hjust = 0.5)) 


#save shit for excelling
exp_deg_pca <- p.PCi[,1:2]
write.csv(exp_deg_pca, "./exp_deg_pca.csv", row.names=TRUE)
...
q <- ggbiplot(p.pca, group=p$pop, labels=NULL, ellipse = TRUE, var.axes=FALSE) +
  ...
  xlab(paste("PC1, ", round(p.pca.proportionvariances[1], 2), "%")) + 
  ylab(paste("PC2, ", round(p.pca.proportionvariances[2], 2), "%")) +
  labs(title="Average Expression Values in Treatment Groups \nPopulation Exposure Interaction Effect") +
  theme(plot.title = element_text(hjust = 0.5)) 

#again save the coordinate for tooling around in gridtopia
ixn_deg_pca <- p.PCi[,1:2]
write.csv(ixn_deg_pca, "./ixn_deg_pca.csv", row.names=TRUE)
...
p$pop <- as.factor(c(rep("ARS", 4), rep("GT", 4), rep("GB", 4), rep("VB", 4)))
p$pop <- factor(p$pop, levels = c("ARS", "GT", "GB", "VB"))
levels(p$pop) <- list("LA-REF"="ARS", "LA-DWH"="GT", "TX-REF"="GB", "TX-SF"="VB")
p$exposure <-c(rep(c("0", "10", "32", "56"), 4))
#p$trt <- y$samples$trt
pcadim <- dim(p)[2]-2
p.pca <- prcomp(p[,1:pcadim])
summary(p.pca)
...
q <- ggbiplot(p.pca, group=p$pop, labels=NULL, ellipse = TRUE, var.axes=FALSE) +
   geom_point(aes(colour=p$pop, shape=p$exp), size=3) + 
  scale_color_manual(name="Population", values=c("dodgerblue4", "deepskyblue1", "goldenrod1", "olivedrab3")) + 
  scale_shape_manual(name="Exposure (HEWAF %)", values=c(19,1,12,8)) + 
  scale_size(guide="none") +
  theme_minimal() +
  xlab(paste("PC1, ", round(p.pca.proportionvariances[1], 2), "%")) + 
  ylab(paste("PC2, ", round(p.pca.proportionvariances[2], 2), "%")) +
...
q <- ggplot(p.PCi, aes(x=PC1, y=PC2, color=pop, 
  ...
  xlab(paste("PC1, ", round(p.pca.proportionvariances[1], 2), "%")) + 
  ylab(paste("PC2, ", round(p.pca.proportionvariances[2], 2), "%")) +
  labs(title="PCA: Average Expression Values in Treatment Groups \nAHR Pathway-Related Genes \nPopulation by Exposure Interaction Effect") +
  theme(plot.title = element_text(hjust = 0.5)) 
...
# all genes regardless of significance threshold 
all_ahr <- gnorm[gnorm$Gene %in% ahr_genes$`Killifish geneID`,]

# only those with p-val < 0.1 for any term: 
ahr_sig <- adj_p_gnorm[adj_p_gnorm$Gene %in% ahr_genes$`Killifish geneID` 
                       & adj_p_gnorm$adj_pvals <= 0.1,]
any_sig <- unique(ahr_sig$Gene)
all_sig_ahr <- all_ahr[all_ahr$Gene %in% any_sig,]

# only pop main or pop-exp interaction:
ixn <- adj_p_gnorm[adj_p_gnorm$cof=="pop_exposure" & 
                       adj_p_gnorm$adj_pvals <= 0.1,]
...
#save for excel
ahr_all_pca <- p.PCi[,1:2]
write.csv(ahr_all_pca, "./ahr_all_pca.csv", row.names=TRUE)

ahr_sig_pca <- p.PCi[,1:2]
write.csv(ahr_sig_pca, "./ahr_sig_pca.csv", row.names=TRUE)
...
q <- ggplot(p.PCi, aes(x=PC1, y=PC2, color=pop, 
  ...
  scale_shape_manual(name="Exposure", values=c(19,1,12,8)) + 
  theme_minimal() +
  stat_ellipse_model(type = "confidence", formula = y ~ x, color = pop, size = 2) +
  xlab(paste("PC1, ", round(p.pca.proportionvariances[1], 2), "%")) + 
  ylab(paste("PC2, ", round(p.pca.proportionvariances[2], 2), "%")) +
  labs(title="PCA: Average Expression Values in Treatment Groups \nAHR Pathway-Related Genes \nPopulation by Exposure Interaction Effect") +
  theme(plot.title = element_text(hjust = 0.5)) 
