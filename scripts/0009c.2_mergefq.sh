...
#SBATCH --mem=16000
#SBATCH -o /home/jajpark/niehs/slurm-log/fqmerge-stout-%j.txt
#SBATCH -e /home/jajpark/niehs/slurm-log/fqmerge-stderr-%j.txt
#SBATCH -J fqmerge
#SBATCH -p high 
#SBATCH -t 24:00:00 
## Modified 6 January, 2017, JP
...
cd $DIR2
for m in `ls *R1*.qc.fq.gz`
do 
	
	name=`echo $m | cut -f 1 -d "_"`
	echo $name
	
	#zcat $name*R1*.qc.fq.gz > ${OUTDIR}/${name}_R1_merged.fq
	zcat $name*R2*.qc.fq.gz > ${OUTDIR}/${name}_R2_merged.fq
