basedir <- "~/niehs/results/htseqcount/exons_by_dbxref"
setwd(basedir)

pat <- "_htseq.count"
htseq.all <- list.files(path = basedir,
	pattern = pat)
...
for (i in 1:length(myfiles) ) {
	...
	cnts <- gsub("_htseq.count", "", myfiles[i])
...
data.all.summary <- data[grep("^GeneID", rownames(data), perl=TRUE, invert=TRUE), ]
...
write.csv(data.all.summary, file = "2017-01-20_htseq_counts_all-summary.csv")

# final merged table
head(data.all, 3)

data.all <- data[grep("^GeneID", rownames(data), perl=TRUE, invert=FALSE), ]
write.csv(data.all, file = "2017-01-20_htseq_counts_all.csv")
