...
files_list <- list.files(path = dir, pattern = "180403_")
files <- file.path(dir, files_list)
filenames <- gsub("180403_", "", files_list)
...
write.table(david, file="~/Documents/NIEHS_LSU_UCD/rnaseq/david_results/180409_clustergenelists.txt", row.names = FALSE, col.names = TRUE, sep="\t", quote=FALSE)

#Compile background genelists by uniprot accession IDs and store one multi-list file: 
bg <- vector("list", 3)
...
##Look at inter-individual variation in non-DEG expression between parent control and parent exposed groups, or plot inviduals on MDS space:
